...
- **Text-editor zero hotkey** — `0` is always a literal digit while free_text, number, or Other editors are open (empty or not). Multi-question Submit jump with `0` only works from option navigation / non-editor modes (`93ebccb` and follow-ups).
- **Other / notes navigation** — Tab closes notes on the same question instead of advancing. Left/Right leave Other and change tabs in multi-question forms.
