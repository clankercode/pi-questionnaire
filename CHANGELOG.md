...
- **Other / notes navigation** — `0` is always a literal digit while the Other editor is open (Submit jump only from option navigation). Tab closes notes on the same question instead of advancing. Left/Right leave Other and change tabs in multi-question forms.
